...
describe "Hotel class" do
  ...
  describe "is room available method" do 
    before do
      ...
      # Date vs DateTime: uses hours, midnight etc
      @date_range = DateRange.new(check_in: Date.new(2019, 9, 10), check_out: Date.new(2019, 9, 15))
      @reservation = Reservation.new(date_range: @date_range, room_number: 1, confirmation_id: nil, total_cost: nil)
      @hotel = Hotel.new(reservation_list:[@reservation]) 
    end
    ...
    it "is room available to book when there are previously held bookings" do 
      expect(@hotel.is_room_available(@date_range, 1)).must_equal false # make everything else like this in the parens
    end
  end
